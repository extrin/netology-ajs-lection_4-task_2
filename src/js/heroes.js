...
const sortHeroes = (heroes) => {
  ...
  if (heroes.some(hero => !isObject(hero))) {
    throw new UserException('Input does not contain objects');
  }
  if (heroes.some(hero => !('health' in hero))) throw new UserException('Heroes have no health');
  if (heroes.some(hero => !isNumber(hero.health))) {
    throw new UserException('Health is not a number');
  }
